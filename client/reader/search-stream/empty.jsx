...
const SearchEmptyContent = React.createClass( {
	...
	render: function() {
		const action = (
			<a className="empty-content__action button is-primary" onClick={ this.recordAction } href="/">{this.translate( 'Back to Following' )}</a>
		);

		const secondaryAction = isDiscoverEnabled()
			? <a className="empty-content__action button" onClick={ this.recordSecondaryAction } href="/discover">
					{this.translate( 'Explore Discover' )}
				</a>
			: null;

		const message = this.translate( 'No posts found for {{query /}} for your language.', {
