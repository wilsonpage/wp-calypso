...
class SearchStream extends Component {
	...
	updateQuery = newValue => {
		...
		if ( ( trimmedValue !== '' && trimmedValue.length > 1 && trimmedValue !== this.props.query ) || newValue === '' ) {
	...
	render() {
		...
		const transformStreamItems = ! query || query === '' ? postKey => ( { ...postKey, isRecommendation: true } ) : identity;
		...
		const suggestionList = initial(
			flatMap( suggestions, suggestionKeyword => [
				<Suggestion suggestion={ suggestionKeyword } source="search" sort={ sortOrder === 'date' ? sortOrder : undefined } />,
				', ',
			] ),
		);

		const documentTitle = translate( '%s ‹ Reader', { args: this.state.title || translate( 'Search' ) } );
			...
			>
				...
				<div className="search-stream__fixed-area" ref={ this.handleSearchBoxMounted }>
					...
					<p className="search-stream__blank-suggestions">
						{suggestions &&
							translate( 'Suggestions: {{suggestions /}}.', {
								components: {
				...
				{query && <ConnectedTerribleSitesResults query={ query } />}
			</Stream>
		);
	}
}

class TerribleSitesResults extends React.Component {
	componentWillMount() {
...
const ConnectedTerribleSitesResults = connect(
	( state, ownProps ) => {
		return {
